...
try {
  if (fs.existsSync(DATASET_PATH)) {
    ...
    console.log(
      `Loaded ${localDataset.length} itineraries from local dataset.`
    );
  } else {
...
if (process.env.GEMINI_API_KEY) {
  ...
  console.warn(
    "GEMINI_API_KEY not found in environment variables. using mock."
  );
}
...
export async function generateItinerary(params) {
  ...
  const datasetItinerary = getFromLocalDataset(
    params.destination,
    params.duration
  );
  if (datasetItinerary) {
...
export async function generateSignExplanation(text) {
  ...
  try {
    const prompt = `You are an ASL interpreter helper. 
        ...
        Include relevant emojis in your description to make it more visual.
...
function getFromLocalDataset(destination, duration) {
  ...
  const match = localDataset.find(
    (r) => r.dest && r.dest.toLowerCase().includes(destination.toLowerCase())
  );

  if (match) {
    console.log(
      `Found dataset match for ${destination} (Row ${match.row_idx})`
    );
